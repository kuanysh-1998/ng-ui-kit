...
{
  @Input() public content!: string | number | TemplateRef<unknown>;
  ...
  @Input() public hideOnOutsideClick:
    | boolean
  ...
  private readonly _popoverContainer!: TemplateRef<any>;
  ...
  private _initTooltip(): void {
    ...
    const viewRef = this.vcr.createEmbeddedView(this._popoverContainer);
  ...
  private _onDocumentClick(event: MouseEvent): void {
    const clickedInsidePopover = this.popoverElement?.contains(
