...
export abstract class Overlay {
  ...
  public setPosition(): void {
    ...
    const spaceBottom =
      window.innerHeight - targetRect.bottom - this.offset - this._gap;
    ...
    const center = {
      horizontal:
        ...
        popoverRect.width / this._two,
      vertical:
        targetRect.top +
        targetRect.height / this._two -
        popoverRect.height / this._two,
    };
    ...
    if (left !== null) {
      if (left < this._gap) {
        ...
        left =
          window.innerWidth -
          Math.min(popoverRect.width, maxWidth || 0) -
          this._gap;
      }
    }
    ...
    if (top !== null) {
      if (top < this._gap) {
        ...
        top =
          window.innerHeight -
          Math.min(popoverRect.height, maxHeight || 0) -
          this._gap;
      }
    }
    ...
    if (this.popoverElement.classList.contains("ng-tooltip")) {
      if (
        ...
        if (left && popoverRect.width < targetRect.width / 2 + 12) {
          left =
            left + targetRect.width / 2 - popoverRect.width + this._gap * 2;
        }

        if (right && popoverRect.width < targetRect.width / 2 + 12) {
          right =
            right + targetRect.width / 2 - popoverRect.width + this._gap * 2;
        }
      }
      ...
      if (
        ...
        if (top && popoverRect.height < targetRect.height / 2 + 12) {
          top =
            top - targetRect.height / 2 + popoverRect.height - this._gap * 2;
        }

        if (bottom && popoverRect.height < targetRect.height / 2 + 12) {
          bottom =
            bottom - targetRect.height / 2 + popoverRect.height - this._gap * 2;
        }
      }
    }
    ...
    const arrow = this.popoverElement.querySelector(
      ".ng-tooltip__arrow"
    ) as HTMLElement;
    ...
    if (arrow) {
      if (
        ...
        if (top)
          this.renderer.addClass(this.popoverElement, "ng-tooltip_bottom");
        if (bottom)
          this.renderer.addClass(this.popoverElement, "ng-tooltip_top");
        ...
        if (right)
          this.renderer.addClass(this.popoverElement, "ng-tooltip_left");
        if (left)
          this.renderer.addClass(this.popoverElement, "ng-tooltip_right");
  ...
  protected addScrollListeners(): void {
    ...
    const windowScrollListener = (event: Event): void => {
      ...
      const inOverlayContent =
        overlayContent && overlayContent.contains(eventTargetNode);
